  ...
  return (
    <div className="space-y-8">
      <header className="space-y-1.5">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-white">{current.label}</h1>
        <p className="max-w-2xl text-sm leading-relaxed text-slate-400">{current.description}</p>
      </header>

      <section
        aria-label="Status pengembangan modul"
        className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/15 bg-white/[0.03] px-6 py-16 text-center"
      >
        <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/5">
          <Clock className="h-7 w-7 text-slate-400" aria-hidden="true" />
        </span>
        ...
        <p className="mt-1.5 max-w-md text-sm leading-relaxed text-slate-400">
          ...
          responsif, dan tema dark sudah tersedia di seluruh bagian aplikasi.
        </p>
      </section>
    </div>
  );
